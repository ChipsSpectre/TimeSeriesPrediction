...
data characteristic to biological measurements. The results for low noise 
($\sigma=0.01$) are given in Table~\ref{tab:mackey_lownoise} and the results for
a higher noise ($\sigma = 0.1$) 
level are given in Table~\ref{tab:mackey_highnoise}.

\begin{table}
  \centering
  \begin{tabular}{c|c|c}
    Architecture & $i.i.d.$ noise & Wiener process noise \\
    f & g & g \\
  \end{tabular} 
  \caption{low}
  \label{tab:mackey_lownoise}
\end{table}
